...
internal sealed class PendingUserRepository : IPendingUserRepository
{
    ...
    public async Task<PendingUser> CreateAsync(string userName, string email, string password, string verificationCode, CancellationToken cancellationToken = default)
    {
        ...
        if (await ExistsByUserNameAsync(userName, cancellationToken))
        {
    ...
    public async Task<PendingUser?> GetByEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        return await _context.PendingUsers.Where(pu => pu.Email == new Email(email)).FirstOrDefaultAsync(cancellationToken);
    }

    public async Task<bool> ExistsByEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        return await _context.PendingUsers.AnyAsync(pu => pu.Email == new Email(email), cancellationToken);
    }

    public async Task<bool> ExistsByUserNameAsync(string userName, CancellationToken cancellationToken = default)
    {
        return await _context.PendingUsers.AnyAsync(pu => pu.UserName == new UserName(userName), cancellationToken);
    }
    ...
    {
        var existingPendingUser = await GetByEmailAsync(email, cancellationToken);
